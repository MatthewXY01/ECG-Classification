...
def main(train_path, test_path, dir_csv):
    print("SVM for ECG Classification!")
    # Load data
    ...
    # Construct SVM classifier and training
    rng = np.random.RandomState(23)
    model = OneVsRestClassifier(svm.SVC(kernel='linear', probability=True, random_state=rng))
    SVM = model.fit(feature_train_norm, label_train)

    train_acc = SVM.score(feature_train_norm, label_train)
    val_acc = SVM.score(feature_val_norm, label_val)
    ...
    print("Loading result = test(neigh, data_test, name_list, MEAN, STD)completed!")
    print("Testing...")
    result = test(SVM, data_test, name_list, MEAN, STD)
